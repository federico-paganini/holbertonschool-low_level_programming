...
char *multiply(char *num1, char *num2)
{
	...
	int result[2000];
	char *res_str;
	int i, j, carry, pos, prod, start = 0;
	...
	for (i = len1 - 1; i >= 0; i--)
	{
		...
		for (j = len2 - 1; j >= 0; j--)
		{
			pos = 1999 - ((len1 - 1 - i) + (len2 - 1 - j));
			prod = (num1[i] - '0') * (num2[j] - '0') + result[pos] + carry;
			result[pos] = prod % 10;
			carry = prod / 10;
		}
		result[pos - 1] += carry;
	}
	while (start < 2000 && result[start] == 0)
		start++; 
	res_str = malloc((2000 - start + 1) * sizeof(char));
	if (!res_str)
	{
		printf("Error\n");
		return (NULL);
